      ...
      <div class="form-group col-md-5">
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                echo '<option value="'. $row["ID_CAT"].'">'. $row["DESC_cat"].'</option>';
            }
        } 
        else {
  ...
  if(isset($_POST['addProduit']))
    {
        ...
        $sql = "select NOM from produit where NOM='".$nomProd."'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0)
        {
        ...
        else
        {
           ...
           //$sql = "INSERT INTO produit (idCategorie,nomProduit,prix,quantiteStock,imageProduit,produtit_panier_standard,qte_ligne_panier_standard)VALUES ($idCat,'$nomProd',$prix,$qteStock,'$imageProduit',$panierStandard,$quantitepanier)";
            ...
            //$sql = "INSERT INTO produit (MATRICULE,ID_CAT,NOM,QTE_MAX,IMAGE)VALUES ('admin',$idCat,'$nomProd',$qteStock,'$imageProduit')";
            $sql = "INSERT INTO `produit` (`ID_PRD`, `MATRICULE`, `ID_CAT`, `NOM`, `QTE_MAX`, `IMAGE`, `prix`, `produtit_panier_standard`, `qte_ligne_panier_standard`) VALUES ('', 'admin', $idCat,'".$nomProd."', $qteStock, '".$imageProduit."',$prix,$panierStandard,$quantitepanier)";
